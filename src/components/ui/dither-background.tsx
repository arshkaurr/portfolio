...
function DitheredWaves({
  ...
  const planeScale = useMemo(
    () => [viewport.width * 1.6, viewport.height * 1.6, 1] as const,
...
function Dither({
  waveSpeed = 0.05,
  waveFrequency = 1.5, // Lower frequency = bigger chunks
  waveAmplitude = 0.6, // Higher amplitude = more contrast
  waveColor = [0.784, 0.529, 0.373],
  colorNum = 3, // Fewer colors = more posterized/chunky
  pixelSize = 4, // Bigger pixels = chunkier look
  disableAnimation = false,
  enableMouseInteraction = true,
  mouseRadius = 0.35
}: DitherProps) {
  const dpr = useMemo(() => (typeof window !== "undefined" ? Math.min(window.devicePixelRatio, 1.8) : 1), []);
  ...
  return (
    ...
    >
      <color attach="background" args={["#e2d4c6ff"]} />
      <DitheredWaves
        waveSpeed={waveSpeed}
...
export function DitherBackground({ className }: DitherBackgroundProps) {
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 z-0 h-full w-full overflow-hidden pointer-events-none",
        className
      )}
      style={{ width: "100%", height: "100%" }}
    >
      {isMounted ? <Dither enableMouseInteraction /> : null}
    </div>
  );
}
